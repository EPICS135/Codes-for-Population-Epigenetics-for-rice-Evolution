#usr/bin/perl -w
#The program can be used for mapping ATAC-seq with PE-reads and generate reports for QC, mapping and IGV.
...
$split_genome_50bp="../rice_genome_split_into_50bp.txt";# modification
#raw reads
...
open(F1,"$ARGV[0].sam") or die "$!";
my @name1=split/\./,$ARGV[1];
$out1=$name1[0]."_tpm.txt";
open(OUT1,">$out1") or die "$!";
print OUT1 "genename","\t","count","\t","normalread","\t","tpm","\n";#文件1
%hash=();
$k=0;
$ii=1;
while(my $line=<F1>){
...
open(F1,$split_genome_50bp) or die "$!";
while(my $line=<F1>){
	chomp $line;
	my @arr=split/\t/,$line;
	$j=0;
...
if($j!=0){
print OUT1 $arr[0],"\t","+","\t",$arr[3],"\t",$arr[4],"\t","hits=",$j,"\t","*	*	*","\t","read=",$j,"\t",$tpm,"\n";
}#文件1
}
